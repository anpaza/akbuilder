...
Z5_DTS_PATCH = build/$(PLATFORM)/kernel/z5-dts.patch
...
PLATFORM.UBOOT_AUTOSCRIPT ?= build/VontarZ5/uboot-script-libreelec.txt
$(OUT)aml_autoscript: $(PLATFORM.UBOOT_AUTOSCRIPT)
	sed -e 's/@BOOTIMG@/$(notdir $(OUT)$(BOOTIMG.FILE))/' $< > $@.tmp
	mkimage -T script -C none -n 'Vontar Z5 custom kernel autoscript' -d $@.tmp $@
	$(call RM,$@.tmp)
